zdoArmaVoice_fnc_commandLookat = {
    params ["_args", "_lookAtPosition", "_units"];
    private _pos = [_args getOrDefault ["position", "lookAt"], _lookAtPosition] call zdoArmaVoice_fnc_resolvePosition;
    (_units apply { _x call BIS_fnc_objectFromNetId }) lookAt _pos
};
["lookat",
"Make units look at a position or direction. Triggers: look there, look at that. For cardinal directions use relative position with distance=100.",
"{position?: Position}",
zdoArmaVoice_fnc_commandLookat] call zdoArmaVoice_fnc_coreRegisterCommand
